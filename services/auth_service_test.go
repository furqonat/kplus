...
func TestAuthServiceSignIn(t *testing.T) {
	...
	if result.Token != expectedResult.Token {
		t.Errorf("expect %v, got %v", expectedResult.Token, result.Token)
	}
...
func TestAuthServiceSignUp(t *testing.T) {
	...
	mock.ExpectExec(`INSERT INTO users (role, phone, email, password) VALUES (?, ?, ?, ?\)`).
		WithArgs(utils.RoleUser, "1234567890", nil, gomock.Any()).
		WillReturnResult(sqlmock.NewResult(1, 1))

	// Mock Exec response for inserting loans
	mock.ExpectExec("INSERT INTO loans (user_id, `limit`, tenor) VALUES (?, ?, ?)").
		WithArgs("1", 100000, 1).WillReturnResult(sqlmock.NewResult(1, 1))
	mock.ExpectExec("INSERT INTO loans (user_id, `limit`, tenor) VALUES (?, ?, ?)").
		WithArgs("1", 200000, 2).WillReturnResult(sqlmock.NewResult(1, 1))
	mock.ExpectExec("INSERT INTO loans (user_id, `limit`, tenor) VALUES (?, ?, ?)").
		WithArgs("1", 500000, 3).WillReturnResult(sqlmock.NewResult(1, 1))
	mock.ExpectExec("INSERT INTO loans (user_id, `limit`, tenor) VALUES (?, ?, ?)").
		WithArgs("1", 1000000, 6).WillReturnResult(sqlmock.NewResult(1, 1))
